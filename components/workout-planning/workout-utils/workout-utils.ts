...
export function mapWorkoutPlanResponseToPhase(
  ...
  return response.phases.map((phase) => {
    ...
    return {
      ...
      orderNumber: phase.orderNumber !== undefined ? phase.orderNumber : 0,
      sessions: phase.sessions.map((session) => {
        ...
        const exercises: Exercise[] =
          sortedExercises?.map((e) => {
            ...
            return {
              ...
              duration: typeof e.duration === "number" ? e.duration : 8,
              ...
              restMin: e.restMin ?? "",
              restMax: e.restMax ?? "",
              customizations: e.customizations ?? e.additionalInfo ?? "",
              notes: e.notes ?? "",
            };
          }) || [];

        const calculatedDuration =
          exercises.reduce((total, ex) => total + (ex.duration || 8), 0) || 0;
        ...
        return {
          ...
          orderNumber:
            session.orderNumber !== undefined ? session.orderNumber : 0,
        };
      }),
    };
  });
...
export const createUpdatePhasesFunction = (
  ...
  return (newPhases: Phase[] | ((prevPhases: Phase[]) => Phase[])) => {
    setPhases((prevPhases) => {
      ...
      const updatedPhases =
        typeof newPhases === "function" ? newPhases(prevPhases) : newPhases;
