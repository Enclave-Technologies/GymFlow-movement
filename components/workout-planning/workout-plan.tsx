...
export default function WorkoutPlanner({
    ...
    phases.forEach((phase) => {
      ...
      phase.sessions.forEach((session) => {
        ...
        session.exercises.forEach((exercise) => {
          const description = exercise.description ?? "(No description)";
  ...
  const handleDuplicatePhase = async (phaseId: string) => {
    // Use the latest phases from the ref
    const currentPhases = latestPhasesRef.current;
    duplicatePhase(phaseId, currentPhases, updatePhases, setHasUnsavedChanges);
  ...
  const confirmDeleteExerciseHandler = async (
    ...
    // Use the latest phases from the ref
    const currentPhases = latestPhasesRef.current;
    updatePhases(deleteExercise(currentPhases, phaseId, sessionId, exerciseId));
    setShowConfirm({ type: null });
    setHasUnsavedChanges(true);
  ...
  const moveSession = async (
    ...
    if (!updatedPhase) {
      ...
      toast.error("An internal error occurred while reordering sessions.");
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.conflict) {
          setConflictError({
            message:
              result.error || "Plan modified by another user during reorder",
          ...
          toast.error(result.error || "Failed to update session order");
          // We already set hasUnsavedChanges to true above
        }
      }
    } catch (error) {
