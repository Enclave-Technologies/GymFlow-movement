...
export function createExerciseHandlers(
  ...
  const handleSaveExercise = async (
    ...
    if (!session) {
      console.error(`Session with ID ${sessionId} not found in phase ${phaseId}`);
      return;
    }

    const exercise = session.exercises.find((e) => e.id === exerciseId);
    if (!exercise) {
      console.error(`Exercise with ID ${exerciseId} not found in session ${sessionId}`);
      return;
    }
    ...
    console.log(`💾 Saving exercise as ${isNewExercise ? 'NEW' : 'EXISTING'} exercise`);
    ...
    try {
      if (props.planId && exerciseData) {
        ...
        toast.success(`Exercise ${actionText} and queued for processing.`, {
          duration: 2000,
        });
      }
    } catch (error) {
  ...
  const confirmDeleteExerciseHandler = async (
    ...
    if (exerciseEditState.isEditingExercise(sessionId, exerciseId) || 
