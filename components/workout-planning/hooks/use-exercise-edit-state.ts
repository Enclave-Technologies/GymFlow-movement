...
export interface UseExerciseEditStateReturn {
  ...
  // Actions
  startCreatingExercise: (sessionId: string, exerciseId: string, originalData?: Exercise) => void;
  startEditingExercise: (sessionId: string, exerciseId: string, originalData: Exercise) => void;
...
export function useExerciseEditState(): UseExerciseEditStateReturn {
  const [exerciseEditState, setExerciseEditState] = useState<ExerciseEditState | null>(null);
  ...
  const startCreatingExercise = useCallback((
    ...
    console.log('🆕 Started creating new exercise:', { sessionId, exerciseId });
  }, []);
  ...
  const startEditingExercise = useCallback((
    ...
    console.log('✏️ Started editing existing exercise:', { sessionId, exerciseId });
  }, []);
   ...
   */
  const isCreatingExercise = useCallback((sessionId: string, exerciseId: string): boolean => {
   ...
   */
  const isEditingExercise = useCallback((sessionId: string, exerciseId: string): boolean => {
    const current = stateRef.current;
    return current !== null && 
           current.sessionId === sessionId && 
           current.exerciseId === exerciseId && 
 ...
 */
export function isCreationState(state: ExerciseEditState | null): state is ExerciseEditState & { isNew: true } {
 ...
 */
export function isEditingState(state: ExerciseEditState | null): state is ExerciseEditState & { isNew: false } {
