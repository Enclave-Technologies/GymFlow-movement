...
const ClientWorkoutHistoryList: React.FC<ClientWorkoutHistoryListProps> = ({
  ...
  const [detailsMap, setDetailsMap] = useState<Record<string, WorkoutDetail[]>>(
    {}
  );
  ...
  const handleExpand = (sessionId: string) => {
    ...
    if (!detailsMap[sessionId]) {
      startTransition(async () => {
        try {
          const details = await fetchWorkoutDetailsBySession(sessionId, userId);
          setDetailsMap((prev) => ({
            ...prev,
            [sessionId]: details,
          }));
  ...
  const handleEditDetail = (
    detailId: string,
    sessionId: string,
    changes: Partial<Omit<WorkoutDetail, "id" | "exerciseName" | "entryTime">>
  ) => {
    startTransition(async () => {
      try {
        await updateWorkoutHistoryEntry(detailId, changes);
        // Refetch the entire session details to ensure we have the latest data
        const updated = await fetchWorkoutDetailsBySession(sessionId, userId);
        setDetailsMap((prev) => {
          const newMap = { ...prev };
            ...
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="flex items-center space-x-4">
                ...
                <button
                  ...
                  onClick={() =>
                    handleDeleteSession(session.workoutSessionLogId)
                  }
              ...
              <div>
                ...
                <div className="text-muted-foreground">
                  {detailsMap[session.workoutSessionLogId]
                    ? detailsMap[session.workoutSessionLogId].reduce(
                        (total, exercise) =>
                          parseFloat(
                            (total + exercise.reps * exercise.weight).toFixed(2)
                          ),
                        0
                      ) + " kg"
                    : "Expand to view"}
            ...
            {expandedSession === session.workoutSessionLogId && (
              <div className="mt-4 space-y-2">
                {(() => {
                  ...
                  const details = detailsMap[session.workoutSessionLogId] ?? [];

                  // Group by exercise name
                  const exerciseGroups: Record<string, WorkoutDetail[]> = {};

                  details.forEach((detail) => {
                    if (!exerciseGroups[detail.exerciseName]) {
                      exerciseGroups[detail.exerciseName] = [];
                    }
                    exerciseGroups[detail.exerciseName].push(detail);
                  });

                  console.log(JSON.stringify(exerciseGroups, null, 2));
                    ...
                    ([exerciseName, exercises]) => (
                      <div key={exerciseName} className="mb-4 last:mb-0">
                        <div className="font-medium text-accent-foreground mb-4 text-lg text-center">
                          {exercises[0].setOrderMarker}. {exerciseName}
                        </div>
                        {exercises.map((exercise, index) => (
                          ...
                          >
                            <div className="text-sm text-accent-foreground">
                              Set {index + 1}: Reps: {exercise.reps}, Weight:{" "}
                              {exercise.weight}
                              kg, Volume:{" "}
                              {(exercise.reps * exercise.weight).toFixed(2)}
                              kg
                              {exercise.notes && (
                            ...
                            <Popover
                              open={editingNote?.id === exercise.id}
                              onOpenChange={(open) => {
                                if (open) {
                                  setEditingNote({
                                    id: exercise.id,
