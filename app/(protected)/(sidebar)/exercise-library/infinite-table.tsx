...
export function InfiniteTable({
  ...
  const { mutate: bulkUpdateStatus } = useMutation({
    mutationFn: async ({
      ...
      try {
        const result = await bulkUpdateExerciseStatus(exerciseIds, approved);
        if (!result.success) {
        ...
        toast.custom(
          () => (
            <div className="bg-background border rounded-lg shadow-lg p-4 max-w-md">
              <h3 className="font-semibold mb-2">Bulk Delete Results</h3>
              <p className="text-sm mb-2">{data.message}</p>
              <div className="mt-2">
                <p className="text-sm font-medium mb-1">Failed to delete:</p>
        ...
        customOperations={[
          {
            ...
            onClick: (rows) => {
              if (rows.length > 0) {
                ...
                bulkUpdateStatus({ exerciseIds, approved: true });
              }
            },
          ...
          },
          {
            ...
            onClick: (rows) => {
              if (rows.length > 0) {
                const exerciseIds = rows.map((row) => row.exerciseId);
                bulkUpdateStatus({ exerciseIds, approved: false });
              }
            },
            showConfirmation: true,
