...
export async function getClientsManagedByUserPaginated(
    ...
    .map((filter: { id: string; value: unknown }) => {
      ...
      switch (id) {
        ...
        case "gender":
          ...
          } else if (genderValue === "NB" || genderValue === "NON-BINARY") {
  ...
  if (sorting.length > 0) {
    orderByColumns = sorting.map((sort) => {
      ...
      switch (id) {
        case "email":
          return isDesc ? desc(Client.email) : sql`${Client.email} asc`;
        case "fullName":
          return isDesc ? desc(Client.fullName) : sql`${Client.fullName} asc`;
        case "registrationDate":
          return isDesc
            ? desc(Client.registrationDate)
            : sql`${Client.registrationDate} asc`;
        case "phone":
          return isDesc ? desc(Client.phone) : sql`${Client.phone} asc`;
        case "gender":
          return isDesc ? desc(Client.gender) : sql`${Client.gender} asc`;
        case "age":
          // Age is calculated, so we sort by dob instead
          return isDesc ? sql`${Client.dob} asc` : desc(Client.dob); // Reverse logic for age
        case "trainerName":
          return isDesc ? desc(Trainer.fullName) : sql`${Trainer.fullName} asc`;
        default:
    ...
    const newClient = await db.transaction(async (tx) => {
      ...
      if (existingUser) {
        ...
        if (clientData.emergencyContactName !== undefined)
          updateData.emergencyContactName = clientData.emergencyContactName;
        if (clientData.emergencyContactPhone !== undefined)
          updateData.emergencyContactPhone = clientData.emergencyContactPhone;
...
export async function searchClientsByNameAction(
  ...
  try {
    const clients = await db
      ...
      .where(
        and(ilike(Users.fullName, searchTerm), eq(Roles.roleName, "Client"))
      )
...
export async function registerInternalUser(userData: {
  ...
  try {
    ...
    try {
      ...
      console.log(`Created Appwrite account for ${userData.role}: ${user_id}`);
    } catch (error) {
      if (error instanceof AppwriteException) {
        if (error.code === 409) {
          return {
...
export async function addRoleToUser(email: string, roleName: string) {
  ...
  try {
    ...
    const existingRole = await db
      ...
      .from(UserRoles)
      .where(
        and(eq(UserRoles.userId, user.userId), eq(UserRoles.roleId, roleId))
      )
      .limit(1);
...
export async function getAllCoaches() {
  ...
  try {
    const coaches = await db
      ...
      .innerJoin(Roles, eq(UserRoles.roleId, Roles.roleId))
      .where(or(eq(Roles.roleName, "Trainer"), eq(Roles.roleName, "Admin")))
