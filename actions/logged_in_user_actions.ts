...
export async function get_logged_in_user() {
    ...
    try {
        ...
        try {
            ...
            // Session likely expired or invalid, clear it
        ...
        try {
            ...
            // If we get an authentication error, the session is invalid
            // Clear the invalid session to prevent future issues
            (await cookies()).delete(MOVEMENT_SESSION_NAME);
            return null;
        }
    } catch (error) {
    ...
    const roles = db_user_with_role.map((user) => user.role).filter(Boolean);
